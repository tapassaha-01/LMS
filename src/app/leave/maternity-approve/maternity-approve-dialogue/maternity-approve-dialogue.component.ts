...
@Component({
  selector: 'app-maternity-approve-dialogue',
  templateUrl: './maternity-approve-dialogue.component.html',
  styleUrls: ['./maternity-approve-dialogue.component.css']
})
export class MaternityApproveDialogueComponent {
  ...
  actionInfo: MaternityLeaveRequest = {};
  files!: FileMetaData[];
  displayedColumns: string[] = ['slno', 'fileName','fileType'];
  dataSource = new MatTableDataSource<FileMetaData>(this.files);
  constructor(public leaveReqServ: MaternityServiceService,
    @Inject(MAT_DIALOG_DATA) public dataf: string,
    // private wfhRequestService: WfhRequestService,
    private _snackBar: MatSnackBar,private maternityRequestService:MaternityServiceService,private leaveRequestService:LeaveRequestService) {
  ...
  ngOnInit() {
    ...
    this.leaveReqServ.getListOfFileMetaData(this.users.id).subscribe((data: FileMetaData[]) => {
      this.files = data.filter(e => "Maternity")
      console.log(data)
  ...
  getNumberOfLeaveTakenByMonth()
  {
      ...
      this.maternityRequestService.getNumberOfLeaveTakenBymonth(this.empId).subscribe(data=>
        {
  ...
  onSubmit(action: string): void {
    if (this.users == null) {
      ...
      if (action === 'Approved' || action === 'Rejected') {
        ...
        this.maternityRequestService.approveLeave(this.actionInfo).subscribe(
          (data: any) => {
  ...
  fileDownload(fileId: string, fileName: string) {
    console.log(fileId)
    this.leaveReqServ.downloadDocument(fileId).subscribe((response: any) => {
